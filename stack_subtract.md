...
# Solve:
```bash
pop rax        ; get top stack value
sub rax, rdi   ; rax = rax - rdi
push rax       ; put result back on stack
```
1. `pop rax` means take the value at the top of the stack, remove it from the stack, and store that value in rax.

2. `push rax` pushes the value stored in rax to the top of the stack.
